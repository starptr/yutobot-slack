...
const getLocalDateNow = () => new Date(new Date().toLocaleString("en-US", {
  timeZone: "America/Los_Angeles"
}));

const wiggler = async app => {
  // Get all urls
  let pfpsUrlsAll = require("./pfpsUrls.json"); // Last minute of execution


  let m = getLocalDateNow().getMinutes() - 1; // Last pfp's index

  let oldIndex = -1; // Push pfp
  ...
  const randomPushPfp = async isDay => {
    let dayStr = isDay ? "day" : "night"; // Choose an index not equal to the last index (this algo makes it impossible to choose last pfp on first iteration)
    // i iterates through pfpsUrls's indicies

    let i = oldIndex + 1;

    if (i >= pfpsUrlsAll[dayStr].length) {
      i = 0; // Shuffle pfpsUrls after going through all
